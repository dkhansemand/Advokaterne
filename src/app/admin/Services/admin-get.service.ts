...
export class AdminGetService {
  ...
  getBlogPost(postId : number){
    return this.http.get(AppConfig.api + '/blog/get/post/' + postId).map( (res : Response) => res.json())
  }
  ...
  getSettings(){
    return this.http.get(AppConfig.api + '/settings/get/all').map( (res : Response) => res.json())
  }
